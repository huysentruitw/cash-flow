...
namespace CashFlow.Query.Repositories
{
    internal sealed class SupplierRepository : ISupplierRepository
    {
        ...
        public async Task<IReadOnlyDictionary<Guid, Supplier>> GetSuppliersInBatch(IReadOnlyList<Guid> supplierIds, CancellationToken cancellationToken)
            => await _dataContext.Suppliers.AsNoTracking().Where(x => supplierIds.Contains(x.Id)).ToDictionaryAsync(x => x.Id, cancellationToken: cancellationToken);
    }
}
